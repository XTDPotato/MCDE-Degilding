...
public final class DegildingScreen extends HandledScreen<DegildingScreenHandler> {
    private static final Identifier TEXTURE =
        Identifier.of("mcde_degilding", "textures/gui/degilding_table.png");
    private static final Identifier MISSING_ENCHANTMENT_ICON =
        Identifier.of(MCDE.MOD_ID, "textures/gui/icons/missing_no.png");
    ...
    protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
        ...
        List<RegistryEntry<Enchantment>> gilded =
            DegildingScreenHandler.getGildedEnchantments(handler.getInputStack());
    ...
    protected void drawForeground(DrawContext context, int mouseX, int mouseY) {
        ...
        List<RegistryEntry<Enchantment>> gilded = DegildingScreenHandler.getGildedEnchantments(stack);
        normalizeScroll(gilded.size());
        boolean materialReady = hasEnoughMaterial();

        for (int row = 0; row < visibleEntries(gilded.size()); row++) {
            int index = scrollOffset + row;
            RegistryEntry<Enchantment> enchantment = gilded.get(index);
            int level = EnchantmentHelper.getLevel(enchantment, stack);
            Text name = Enchantment.getName(enchantment, Math.max(level, 1)).copy().formatted(Formatting.GOLD);
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
        List<RegistryEntry<Enchantment>> gilded =
            DegildingScreenHandler.getGildedEnchantments(handler.getInputStack());
        if (isOverList(mouseX, mouseY) && gilded.size() > VISIBLE_ENTRIES && verticalAmount != 0.0D) {
            cancelPendingRemoval();
            scrollOffset = MathHelper.clamp(
                scrollOffset - (int) Math.signum(verticalAmount),
                0,
                maxScroll(gilded.size())
            );
            return true;
        }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        for (int row = 0; row < visibleEntries(gilded.size()); row++) {
            ...
            RegistryEntry<Enchantment> enchantment = gilded.get(scrollOffset + row);
            if (!hasEnoughMaterial()) {
                context.drawTooltip(
                    textRenderer,
                ...
                context.drawTooltip(
                    textRenderer,
                    Enchantment.getName(enchantment, Math.max(level, 1)),
                    mouseX,
                    mouseY
                );
            }
    ...
    private static Identifier getEnchantmentIcon(RegistryEntry<Enchantment> enchantment) {
        Identifier enchantmentId = enchantment.getKey()
            .map(key -> key.getValue())
            .orElse(null);
        if (enchantmentId == null) {
            return MISSING_ENCHANTMENT_ICON;
        }
