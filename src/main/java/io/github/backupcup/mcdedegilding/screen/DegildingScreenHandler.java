...
public final class DegildingScreenHandler extends ScreenHandler {
    ...
    public static List<net.minecraft.util.Identifier> getGildedEnchantments(ItemStack stack) {
        return EnchantmentSlots.fromItemStack(stack)
            .filter(EnchantmentSlots::hasGilding)
            .map(slots -> slots.getGildingIds().stream()
                .sorted(Comparator.comparing(net.minecraft.util.Identifier::toString))
                .toList())
            .orElseGet(List::of);
    }
    ...
    public boolean onButtonClick(PlayerEntity player, int id) {
        ...
        List<net.minecraft.util.Identifier> gildedEnchantments = getGildedEnchantments(stack);
        ...
        net.minecraft.util.Identifier selectedId = gildedEnchantments.get(id);
        Enchantment selected = Registries.ENCHANTMENT.get(selectedId);
        if (selected == null) {
            return false;
        }
        EnchantmentSlots slots = EnchantmentSlots.fromItemStack(stack).orElse(null);
        if (slots == null || !slots.hasGilding(selectedId)) {
            return false;
        }

        var enchantments = new HashMap<>(EnchantmentHelper.get(stack));
        enchantments.remove(selected);
        EnchantmentHelper.set(enchantments, stack);
        slots.removeGilding(selectedId);
        slots.updateItemStack(stack);
